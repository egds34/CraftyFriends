...
import { VoteSite } from "@/components/home/voting-section"

import { getB2Images } from "@/app/actions/image-management"

async function getBannerImages() {
  // 1. Try fetching from B2
  const b2Result = await getB2Images("banner")
  if (b2Result.success && b2Result.images.length > 0) {
    return b2Result.images.map(img => img.url)
  }
...
async function getCommunityImages() {
  // 1. Try fetching from B2
  const b2Result = await getB2Images("featured")
  if (b2Result.success && b2Result.images.length > 0) {
    return b2Result.images.map(img => img.url)
  }
  ...
  try {
    const files = await fs.readdir(communityDir)
    return files
      .filter(file => /\.(png|jpg|jpeg|webp)$/i.test(file))
      .sort(() => Math.random() - 0.5)
      .map(file => `/api/images/community/${file}`)
  } catch (error) {
    // console.error(`Failed to read community images from ${communityDir}`, error)
    return []
  }
}
...
export default async function Home() {
  ...
  const updatesResult = await getUpdates(0, 3)
  ...
  const latestUpdates = updatesResult.success && updatesResult.data ? updatesResult.data : []
